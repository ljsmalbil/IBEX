...
#         super().__init__()
# #     def __init__(self, x_dim, t_dim, t_dim_latent, z_dim, y_dim, hidden_dim=512, hidden_dim_t=16, attn_dim=64):
...
#             nn.Linear(hidden_dim_t, 1)  # Assuming t_dim = 1
#         )

#         self.attention = Attention(z_dim, t_dim_latent, attn_dim)

#         self.decoder_y = nn.Sequential(
#             MLPBlock(z_dim + attn_dim, hidden_dim),
#             MLPBlock(hidden_dim, hidden_dim),
...
#         spline_feats = self.spline_encoder(t)  # spline basis for t

#         t_latent = self.encoder_t(spline_feats)  # latent t features
...
#         y_pred = self.decoder_y(z_attn)  # y | (z, t_latent via attn)
...
# #         self.encoder_t = nn.Sequential(
# #             MLPBlock(t_dim, hidden_dim_t),
# #             nn.Linear(hidden_dim_t, t_dim_latent)
# #         )

# #         self.decoder_t = nn.Sequential(
# #             MLPBlock(z_dim, hidden_dim_t),
# #             nn.Linear(hidden_dim_t, t_dim)
# #         )

# #         # Attention mechanism over (z, t_latent)
# #         self.attention = Attention(z_dim, t_dim_latent, attn_dim)

# #         # Decoder for Y uses both z and attention output
# #         self.decoder_y = nn.Sequential(
# #             MLPBlock(z_dim + attn_dim, hidden_dim),
# #             MLPBlock(hidden_dim, hidden_dim),
# #             nn.Linear(hidden_dim, y_dim)
...
# #     def forward(self, x, t):
# #         z = self.encoder_z(x)               # z | x
# #         t_latent = self.encoder_t(t)        # t_latent | t
# #         t_logits = self.decoder_t(z)        # reconstruct t from z

# #         attn_out = self.attention(z, t_latent)  # attention(z, t_latent)
# #         z_attn = torch.cat([z, attn_out], dim=1)


# #         y_pred = self.decoder_y(z_attn)     # y | (z + attention over t_latent)

# #         return z, t_logits, y_pred


# class CCS_Counterfactual_Net(nn.Module):
#     def __init__(self, x_dim, t_dim_latent, z_dim, y_dim, hidden_dim=512, hidden_dim_t=16, attn_dim=64, use_attention=True):
#         super().__init__()
